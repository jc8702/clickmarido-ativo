...
type RouteParams = { params: Promise<{ serviceOrderId: string }> };

// POST /api/purchase-orders/from-service-order/[serviceOrderId] - Criar OC a partir de Ordem de Serviço
  ...
  try {
    ...
    if (pieceItems.length === 0) {
      return NextResponse.json(
        { error: 'A ordem de serviço (orçamento correspondente) não possui itens do tipo "PEÇA" cadastrados.' },
        { status: 400 }
      );
    }
    ...
    const itemsData = pieceItems.map((item) => {
      ...
      return {
        ...
        taxAmount: 0,
        subtotal: itemSubtotal,
        notes: `Importado da Ordem de Serviço ${serviceOrder.number}`,
      };
    });

    const orderNumber = await generateOrderNumber();

    // Obter endereço de entrega a partir do endereço da OS ou do cliente
    let deliveryAddress = serviceOrder.address;
    if (!deliveryAddress) {
      try {
        const addresses = serviceOrder.customer.addresses as any[];
    ...
    const purchaseOrder = await prisma.purchaseOrder.create({
      data: {
        ...
        status: 'rascunho',
        deliveryAddress,
        requestedBy: 'Importação Automática OS',
        subtotal,
        totalAmount: subtotal,
        internalNotes: `Ordem de compra gerada automaticamente a partir da Ordem de Serviço ${serviceOrder.number}.`,
        items: {
          create: itemsData,
        },
        events: {
          create: {
            type: 'criacao',
            description: `Ordem de compra criada a partir da Ordem de Serviço ${serviceOrder.number}.`,

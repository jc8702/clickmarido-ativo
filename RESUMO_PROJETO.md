...
- **24/06/2026 - 19:13**: Melhoria no PDF e no anexo do chat: Otimizada a renderização para que o PDF de orçamento fique exato no tamanho A4 sem ser cortado no rodapé e mantendo obrigatoriamente a cor branca/clara. Implementado recurso no chat do WhatsApp web para escolher o arquivo localmente, ver a prévia com botão de cancelar e inserir uma legenda junto com o arquivo antes do envio.
- **24/06/2026 - 18:59**: Correção crítica no Chat do WhatsApp: O endpoint utilizado para envio de documentos (`/message/sendDocument`) não existe na Evolution API v1/v2. Substituído em todo o código por `/message/sendMedia` que é o endpoint oficial, garantindo o envio correto do PDF do orçamento sem falhas silenciosas.
